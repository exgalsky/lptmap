...
memfile=open('pure-py_memory_profile.log','w+')

@profile(stream=memfile)
def light_cone():
    ...
    xaxis = np.arange(0, grid_nside, dtype=np.int16)
    yaxis = np.arange(0, grid_nside, dtype=np.int16)
    zaxis = np.arange(0, grid_nside, dtype=np.int16)
    ...
    def lagrange_mesh(x_axis, y_axis, z_axis, trans_vec, Dgrid_in_Mpc):
        return np.meshgrid( np.float32((x_axis + 0.5 + trans_vec[0]) * Dgrid_in_Mpc), np.float32((y_axis + 0.5 + trans_vec[1]) * Dgrid_in_Mpc), np.float32((z_axis + 0.5 + trans_vec[2]) * Dgrid_in_Mpc), indexing='ij')


    def comoving_q(x_i, y_i, z_i, trans_vec, Dgrid_in_Mpc):
        return np.sqrt(x_i**2. + y_i**2. + z_i**2.)
    ...
    def read_displacement(filename):
        return np.asarray(np.fromfile(filename, count=grid_nside * grid_nside * grid_nside, dtype=np.float32).reshape((grid_nside, grid_nside, grid_nside)), dtype=np.float32)
    ...
    for translation in origin_shift:
        ...
        lagrange_grid = comoving_q(grid_qx, grid_qy, grid_qz, translation, lattice_size_in_Mpc)    # 4 : 22

        t6 = time() ; print("Lagrangian comoving distance grid took", t6 - t5, "s ")
        redshift_grid = cosmo_wsp.comoving_distance2z(lagrange_grid)      # 4 : 26

        t7 = time() ; print("Redshift took", t7-t6, "s ")
        # Compute healpix pixel grid from Lagrangian x, y, z values
        ipix_grid = hp.vec2pix(nside, grid_qz, grid_qy, grid_qx)    # 8 : 38

        t8 = time() ; print("HPX pixel grid (Lagrangian) took", t8-t7, "s ")

        kernel_sphere = np.where((lagrange_grid >= chimin) & (lagrange_grid <= chimax), lensing_kernel_F(lagrange_grid, redshift_grid), 0.)      # 4 : 42

        t9 = time() ; print("Kernel grid (Lagrangian) took", t9-t8, "s ")

        skymap += np.asarray(np.histogram(ipix_grid, bins=npix, range=(-0.5,npix-0.5), weights=-kernel_sphere, density=False)[0])      
        del kernel_sphere, ipix_grid         # -20 : 22

        t10 = time() ; print("Project to healpix (Lagrangian) took", t10-t9, "s ")

        growth_grid = cosmo_wsp.growth_factor_D(redshift_grid)        #   4 : 30

        t11 = time() ; print("Growth took", t11-t10, "s ")

        if not store_displacements: grid_sx = read_displacement(sxfile)
        grid_Xx = euclid_i(grid_qx, grid_sx, growth_grid, lattice_size_in_Mpc, translation[0])     # 4 : 34
        del grid_qx
        if not store_displacements: del grid_sx

        if not store_displacements: grid_sy = read_displacement(syfile)
        grid_Xy = euclid_i(grid_qy, grid_sy, growth_grid, lattice_size_in_Mpc, translation[1])     # 4 : 38
        del grid_qy
        if not store_displacements: del grid_sy

        if not store_displacements: grid_sz = read_displacement(szfile)
        grid_Xz = euclid_i(grid_qz, grid_sz, growth_grid, lattice_size_in_Mpc, translation[2])     # 4 : 42
        ...
        ipix_grid = hp.vec2pix(nside, grid_Xz, grid_Xy, grid_Xx)   # 8 : 50 
...
t16 = time() ; print("Job completion took", t16-t0, "s ")
