...
void SetRedshift2RadiusTable(float h, float Omegam, float Omegal, double *table){
  ...
  table[0] = r0 * ZTABLE_INITIAL ; // assuming ZTABLE_INITIAL ~ 0 

  for(int bin=1;bin<NZTABLE;bin++){
    double z  = ZTABLE_INITIAL + bin*dztable;
...
void SetRadius2RedshiftTable(float h, float Omegam, float Omegal, double *table){
  ...
  table[0] = RTABLE_INITIAL / r0 ; // assuming RTABLE_INITIAL ~ 0 

  for(int bin=1;bin<NRTABLE;bin++){
    double r  = RTABLE_INITIAL + bin*drtable;
...
void SetRedshift2TauTable(float h, float Omegab, float Omegam, double *table){
  ...
  float thompson = 6.65e-25; // cm^2
  float YHe = 0.25;
  float hubble0 = 100./3.086e19*h; // 1/s
  float ne0=Omegab*h*h*1.88e-29/1.67e-24; // 1/cm^3
  float c=3e10; // cm/s
  
  float dtau0=thompson*ne0; // 1/cm
  ...
  for(int i=1;i<NZTABLE-1;i++){
    ...
    float dtaudchi = dtau0*pow((1+z),2)*(1-YHe+NHe*YHe/4); // 1/cm
      
    float H = hubble0*sqrt(Omegam*pow((1+z),3)+1-Omegam); // 1/s

    float dtau = dtaudchi * c / H * dztable; 
...
void SetRedshift2WISWTable(float h, float Omegam, double *table){
  ...
  for(int i=0;i<NZTABLE-1;i++){
    ...
    float z   = ZTABLE_INITIAL + i*dztable;
    float wISW = 3./2.*Omegam*(1+z)/r0/r0/r0*(fofz(Omegam,z)-1); 
    table[i] = wISW;
...
void SetRedshift2WKappaTable(float h, float Omegam, float Omegal, float SourceRedshift, double *table, double *Redshift2RadiusTable){
  ...
  float chi0 = r0 * ZTABLE_INITIAL ; // assuming ZTABLE_INITIAL ~ 0 

  for(int bin=1;bin<nz;bin++){
    double z  = bin*dztable;
    double dr = r0 * dztable / sqrt(Omegam*pow((1+z),3)+Omegal);
    chi0 += dr;
  }
...
//////////////// OmegamOfz //////////////////
float fofz(float Omegam, float z){
  return pow(OmegamOfz(Omegam,z),0.6);
}

//////////////// OmegamOfz //////////////////
float OmegamOfz(float Omegam, float z){
  return Omegam * pow((1+z),3)/(Omegam*pow((1+z),3)+1-Omegam);
}
